...
emissions_long = pd.melt(emissions, id_vars = "country", var_name = "year", value_name = "emissions")
emissions_long
```
...
```{python}
emissions_long_1990_2022 = emissions_long.query("year.between(1990,2022)")
...
```{python}
emissions_long_subset = emissions_long_1990_2022.query("country == ['Australia','Denmark'   ,'China','Libya','USA']")
...
```{python}
fig_chart = px.line(emissions_long_subset, x='year',y='emissions',color='country',title="Plot of Emissions over the years by country")
fig_chart
```
